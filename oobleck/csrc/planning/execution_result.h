...
public:
  LayerExecutionResult(const int layer_index, const double forward,
      ...
      : layer_index_(layer_index), forward_(forward), backward_(backward),
  ...
  std::string to_string() const {
    return "LayerExecutionResult[" + std::to_string(layer_index_) +
           "] with forward: " + std::to_string(forward_) +
...
public:
  StageExecutionResult(
      ...
      const std::tuple<int, int> &layer_indices, const int num_gpus,
      const int node_type_idx = 0)
  ...
  std::string to_string() const {
    ...
    return "StageExecutionResult[" + std::to_string(first_layer_index) + ":" +
           ...
           " forward: " + std::to_string(forward_) +
           ", backward: " + std::to_string(backward_);
  }
  ...
  DCExecutionResult(const std::shared_ptr<DCExecutionResult> left,
    ...
    int num_kstar_stage_microbatch =
        (num_batches == 0)
            ...
            : num_batches - (left->stages_.size() + right->stages_.size()) +
                  kstar_ + 1;
...
using CacheMap = oneapi::tbb::concurrent_unordered_map<
    ...
    oobleck::DCExecutionResult::KeyHash, oobleck::DCExecutionResult::KeyEqual>;
